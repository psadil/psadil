...
knitr::opts_chunk$set(echo = FALSE, tidy = TRUE, warning = FALSE, message = FALSE, cache = TRUE)
library(magrittr)
...
make_gabor <- function(f=0.1,
  ...
  d <- tibble::as_tibble(M, .name_repair = "universal") %>%
...
That the recent past influences current perception can be demonstrated easily. If you stare at Figure \@ref(fig:tae), you might observe that the Gabor has a bend immediately after changing orientations. The bend lasts for a moment, then straightens. But the bend is an illusion. While tracking Figure \@ref(fig:tae), the visual system allows for a momentary bias. Usefully, the bias is sensitive to experimental manipulation. Figure \@ref(fig:tae2) shows the same Gabor with the same orientations, but the Gabor also moves. The movement largely eliminates the bending. The sensitivity of such biases to different experimental manipulations enables researchers to study how the visual system balances new information against the recent past. 

```{r, tae, fig.cap="A Gabor alternates between two orientations."}
...
```{r, tae2, fig.cap="A Gabor alternates between two orientations, appearing in a different location with each orientation."}
...
gganimate::animate(p2, nframe=2, fps=1/3)
# gganimate::anim_save(filename="static/img/tae2.gif", animation=anim2)
# knitr::include_graphics('/img/tae2.gif')
```

A closely effect is called _serial dependence_. Serial dependence occurs when participants report the orientations of sequentially presented, tilted Gabors [@fischer2014]. A visual mask to reduces the strong aftereffects present in Figures \@ref(fig:tae) and \@ref(fig:tae2) [Figure \@ref(fig:gabor)]^[The timing and spacing of this figure does not quite match a typical experiment. For example, participants take a few seconds to respond, so the amount of time between Gabors in this figure is too short.]. Even without the aftereffect, the perceptual-decision about one Gabor affects the perceptual-decision about the next; participants report orientations that consistently err toward the orientation of the most recently seen Gabor. Reports on the magnitude of the effect vary, but the error has an average maximum of less than a few degrees. However, serial dependence is affected by different manipulations than that the demonstration of Figures \@ref(fig:tae) and \@ref(fig:tae2). For example, it appears insensitive to the location of the Gabors. This bias may therefore provide a unique way to study how current perception is not only biased by but toward the recent past.

```{r, gabor, fig.cap="Differently oriented Gabors presented with interspersed noise masks.."}
n_frames <- 2
