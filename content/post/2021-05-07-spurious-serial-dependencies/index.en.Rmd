...
```{r, oblique, fig.cap="The simulated data exhibit a clear oblique effect."}
d %>%
...
```{r, spurious, fig.cap="Errors do not depend on the previous orientation, but they do depend on the previous response. Since the data were generated without that dependence, it must be spurious."}
...
```{r, spuriouscol, fig.cap="Same figure as above, but colored based on the magnitude and direction of the oblique effect."}
d %>%
  pivot_longer(
    cols=c(orientation_diff, response_diff), 
    names_to = "covariate",
...
```{r, dep, fig.cap="When the previous orientation is 22.5 clockwise, the current orientation tends to be either 22.5 or 112.5, which is when the oblique effect causes maximal error."}
...
```{r, currbyprev, fig.cap="The current orientation is unrelated to the previous orientation, but there is a dependency on the previous response. "}
d %>%
  pivot_longer(
    cols = c(orientation_diff, response_diff), 
    names_to = "covariate",
