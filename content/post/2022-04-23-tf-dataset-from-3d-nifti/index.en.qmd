...
A recent project required sending brain images to TensorFlow. Unfortunately, the data exceeded memory and so during training would need to be read from the disk. Poking around the TF documentation, it seems like a recommended way to do this is to store the images as a [TFRecord](https://www.tensorflow.org/tutorials/load_data/tfrecord). The steps for doing that are collected in this gist[^src].

[^src]: Inspiration for writting came from responding to [this question on NeuroStars](https://neurostars.org/t/tensorflow-issue-when-trying-to-use-nibabel-in-dataset/22410/2), and also from an urge to try a python-based post.

There are five main steps

1.  load the images as numpy array,
2. perform an preprocessing (e.g., resizing, masking),
3. serialize the preprocessed images and store with their labels (all in memory) as a [`tf.train.Example`](https://www.tensorflow.org/api_docs/python/tf/train/Example),
4. store (on dist) the examples as [TFRecord](https://www.tensorflow.org/tutorials/load_data/tfrecord), and finally
5. create a [`tf.Data.Dataset`](https://www.tensorflow.org/api_docs/python/tf/data/Dataset) pipeline that serves the examples to the model.
...
Here's where tensorflow starts. As those that came before have always done, [we'll rely on these incantations](https://www.tensorflow.org/tutorials/load_data/tfrecord).
...
Those functions seem to help ensure that the serialized data ends up with appropriate types. But the full steps involve serialzing both the image and the label together. 
...
At this point, we have functions for preprocessing the images, serializing them, and packing them together with their labels into [`tf.train.Example`s](https://www.tensorflow.org/api_docs/python/tf/train/Example). This is all for converting the data into a format that can then be more easily written to and read from the disk[^serialize]

[^serialize]: This serializing stuff is spooky black magic. I'm going to skip over those details and instead leave this reference https://blog.djnavarro.net/posts/2021-11-15_serialisation-with-rds/. 

## Write serialized examples as TFRecords

So far, everything has been in memory. Next comes a function that performs the above steps sequentially on several examples, and along the way writing to[`TFRecord`s](https://www.tensorflow.org/tutorials/load_data/tfrecord).[^serialize2] 

[^serialize]: It seems to me that this storage on disk involves a *second* serialization step (where the first was done by `tf.io.serialize_tensor` in `serialize_example`). Even so, I assume the roundtrip isn't so much of a big deal, considering that the data can be served to the model in parallel. 
...
That dataset, `ds`, can now be passed to methods of the `tf.keras.Model` , like [`tf.keras.Model.fit()`(https://www.tensorflow.org/api_docs/python/tf/keras/Model#fit)]! That dataset was our goal in the gist.
...
The dimensions of the labels are easy, a 1d array corresponding to the number of elements within the batch (i.e., the `batch_size` variable)
