...
great_circ <- function(
  ...
  if (!sym) {
    ...
    cw <- great_circ(reference_angle + out, phi_1, range_dist, max_diff, TRUE)
    ccw <- great_circ(reference_angle - out, phi_1, range_dist, max_diff, TRUE)
...
d2 <- d |>
  ...
  mutate(
    channel = if_else(str_detect(Population, "Integrating"), 0, channel),
...
d3 <- d2 |>
  ...
  mutate(y = y + rnorm(n(), 0, .1), y = map_dbl(y, ~ max(.x, 0)))
...
b <- d |>
  mutate(
    beta = 1 + CircStats::dvm(pi / 4, mu, 2),
...
c <- d |>
  mutate(
    mu = mu +
      dog(great_circ(mu, pi / 4, 2 * pi, pi, FALSE), a = 2, b = 1 / (.2 * pi)),
