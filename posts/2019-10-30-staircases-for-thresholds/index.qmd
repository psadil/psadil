...
Staircase <- R6Class(
  ...
  public = list(
    ...
    do_trial = function(participant) {
      ...
      self$tested_contrast <- c(self$tested_contrast, self$current_contrast)
      self$trials <- c(
        self$trials,
        ...
        if (self$pest) {
          ...
          if (
            ...
            if (self$run_number > 1) {
              ...
              if (godown) {
                self$current_step <- abs(self$current_step) * -1
              } else {
                self$current_step <- abs(self$current_step)
              }
            }
          ...
          if (goup) {
            self$current_contrast <- self$current_contrast +
              abs(self$current_step)
          } else {
            self$current_contrast <- self$current_contrast -
              abs(self$current_step)
          }
    ...
    do_experiment = function(participant) {
      while (
        (abs(self$current_step) > self$stopping_step_size) &
...
tibble::tibble(
  Intensity = seq(-3, 3, length.out = 1000),
...
tibble::tibble(Intensity = seq(-4, 4, length.out = 1000)) |>
  ...
  dplyr::mutate(
    positive = pnorm(Intensity, mean = mu),
    Threshold = factor(mu)
  ) |>
