...
dog0 <- function(x, sigma = 1) {
  return(-x * exp(-(x^2) / (2 * sigma^2)) / (sqrt(2 * pi) * sigma^3))
}

dog <- function(x, a = 1, b = 1 / 20, c = sqrt(2 * exp(1))) {
...
b <- tibble::tibble(x = seq(-60, 60, length.out = 1000)) |>
  tidyr::crossing(Amplitude = c(-1, -0.5, 0.5, 1), Width = c(10, 20, 30)) |>
  dplyr::mutate(
...
d |>
  ...
  ggplot2::geom_line(
    ggplot2::aes(y = value, linetype = model, color = model),
    size = 2
  ) +
  ggplot2::scale_color_viridis_d(option = "inferno", end = 0.8, begin = 0.5) +
  ggplot2::scale_x_continuous(
    breaks = c(-0.75, 0, 0.75),
    labels = c("Large Counterclockwise", "0", "Large Clockwise"),
    name = "Orientation on previous trial relative to current"
  ) +
  ggplot2::scale_y_continuous(
    breaks = c(-4, 0, 4),
    labels = c("Counterclockwise", "0", "Clockwise"),
