...
# imager has no osx-arm64 build on conda-forge, so the image is loaded as a plain
# [x, y, channel] array and shifted here. This is imager::imshift with
# boundary = 2, which wraps the content around periodically.
grating2 <- aperm(png::readPNG("grating.png"), c(2, 1, 3))

imshift <- function(im, delta_x, delta_y) {
  nx <- dim(im)[1]
  ny <- dim(im)[2]
  im[
    ((seq_len(nx) - 1L - as.integer(delta_x)) %% nx) + 1L,
    ((seq_len(ny) - 1L - as.integer(delta_y)) %% ny) + 1L,
    ,
    drop = FALSE
  ]
}
