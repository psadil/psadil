...
# gifski has no osx-arm64 build on conda-forge, so gifs come from magick instead.
# magick's own fps handling requires a whole-number centisecond delay, which the
# fractional fps below are not, so give it the delay directly.
options(gganimate.renderer = function(frames, fps) {
...
make_gabor <- function(f = 0.1, theta = 0, res = 5) {
  sigma_x <- 7.0
  sigma_y <- 7.0
  radius <- 20
  r2 <- radius * 2
...
p1 <- tibble::tibble(theta = seq(0, pi / 4, length.out = 2), frame = 1:2) |>
...
p2 <- tibble::tibble(theta = seq(0, pi / 4, length.out = 2), frame = 1:2) |>
  dplyr::mutate(gab = purrr::map(theta, ~ make_gabor(theta = .x, res = 13))) |>
  tidyr::unnest(cols = gab) |>
  ggplot2::ggplot(ggplot2::aes(x = x, y = y)) +
  ggplot2::geom_raster(ggplot2::aes(fill = value), show.legend = FALSE) +
...
n_frames <- 2
p <- tibble::tibble(
  theta = seq(0, pi / 2, length.out = n_frames * 2),
  frame = 1:(n_frames * 2),
  ...
  tidyr::unnest(cols = gab) |>
  dplyr::mutate(
    value = dplyr::if_else(noise, runif(dplyr::n(), min = -1, max = 1), value)
  ) |>
  ggplot2::ggplot(ggplot2::aes(x = x, y = y)) +
  ggplot2::geom_raster(ggplot2::aes(fill = value), show.legend = FALSE) +
  ggplot2::scale_fill_viridis_c(option = "inferno") +
